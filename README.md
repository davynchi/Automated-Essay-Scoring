...
Данные делятся на несколько фолдов (определяемых параметром `n_folds` в конфиге
в папке `conf`). Разделение происходит с помощью Multilabelstratifiedkfold по
лэйблам prompt name (flag) и score (1...6). Модель обучается на всех фолдах,
кроме выбранного, и валидируется сначала на выбранном фолде с flag = 1, а во
второй раз на фолде с флагом flag == 0.
...
- [ ] Отрефакторить submission
- [x] Добавить MLFlow
- [ ] Добавить поддержку kaggle и kaggle/tmp
- [ ] Дописать readme.md
- [ ] Изменить поведение логгера
- [ ] Добавить Lightning, add mlflow again
- [ ] Еще лдин рефакторинг -- пересмотр своего кода
