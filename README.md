...
- `skip_preprocessing: bool = False`
  ...
  должны лежать по пути `CACHED_DATA_DIR = "./cached_data"`.
...
- `delete_model_data_after_training: bool = False`
  ...
  Если `True`, удаляет все пути, в которых хранились данные, не используемые для
  инференса.
...
`-e CUDA_VISIBLE_DEVICES=your_cuda_device_num \`
...
- [x] Поменять расположение best_ensemble_weights, Добавить удаление бесполезных
      для инференса данных, поменять положение cached_data, submission.csv
- [x] Убрать скоры в test.csv, разделив на два файла, добавить подсчет
      итогового, финального скора
- [x] Разделить файлы для скачивания в dvc на train и infer и добавить отдельное
...
- [ ] Сделать правки внутри самой модели
